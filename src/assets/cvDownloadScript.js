...
import { capitalizeWords, formatDate, formatHarvardAuthors, getAllAuthorsName, getYear } from './utilityFunctions';
...
function formatCommittees(title, committees) {
    ...
    if (!_.isEmpty(committees)) {
        ...
        committees.forEach(committee => {
            data.push({
                text: `${committee.start_date ? formatDate(committee.start_date) : ' N/A '} - ${committee.isContinue ? "continue" : committee.end_date ? formatDate(committee.end_date) : ' N/A '}`,
                style: 'dates'
            })
            data.push({
                style: 'committee',
...
function formatCouncils(title, councils) {
    ...
    if (!_.isEmpty(councils)) {
        ...
        councils.forEach(council => {
            data.push({
                text: `${council.start_date ? formatDate(council.start_date) : ' N/A '} - ${council.isContinue ? "continue" : council.end_date ? formatDate(council.end_date) : ' N/A '}`,
                style: 'dates'
            })
            data.push({
                style: 'council',
...
function formatMemberships(title, memberships) {
    ...
    if (!_.isEmpty(memberships)) {
        ...
        memberships.forEach(membership => {
            data.push({
                text: `${membership.start_date ? formatDate(membership.start_date) : ' N/A '} - ${membership.isContinue ? "continue" : membership.end_date ? formatDate(membership.end_date) : ' N/A '}`,
                style: 'dates'
            })
            data.push({
                style: 'membership',
...
function formatAwards(title, awards_honors) {
    ...
    if (!_.isEmpty(awards_honors)) {
        ...
        awards_honors.forEach(award => {
            data.push({
                text: `${award.award_date ? formatDate(award.award_date) : ' N/A '}`,
                style: 'dates'
            })
            data.push({
                style: 'award',
        ...
        conferences.forEach(conference => {
            ...
            data.push({
                ...
                text: [
                    ...
                    conference.start_date ? ' (' + getYear(conference.start_date) + ') ' : '',
                    conference.title,
                    conference.name,
                    !_.isEmpty(conference.city) ? ', ' + conference.city : '',
                    conference.country ? ', ' + conference.country.label : '',
                    !_.isEmpty(conference.page_range) ? ', pages ' + conference.page_range : '',
                    !_.isEmpty(conference.summary) ? '\Conference Summary\n' + conference.summary : ''
                ]
            })
        ...
        journals.forEach(journal => {
            ...
            data.push({
                ...
                text: [
                    ...
                    journal.published_on ? ' (' + getYear(journal.published_on) + ') ' : '',
                    journal.title,
                ...
                .forEach(publication => {
                    ...
                    data.push({
                        ...
                        text: [
                            ...
                            publication.publication_date ? ' (' + publication.publication_date.substring(0, 4) + ') ' : '',
                            publication.title,
                            !_.isEmpty(publication.page_range) ? ', pages ' + publication.page_range : '',
                            !_.isEmpty(publication.publication_event) ? ', ' + publication.publication_event : '',
                            !_.isEmpty(publication.publication_venue) ? ', ' + publication.publication_venue : '',
                            !_.isEmpty(publication.edition_volume) ? ', Edition - ' + publication.edition_volume : '',
    ...
    if (!_.isEmpty(editorial_experience)) {
        ...
        editorial_experience.forEach((experience, index) => {
            ...
            data.push(...[
                ...
                {
                    text: `${experience.start_date ? formatDate(experience.start_date) : ' N/A '} - ${experience.isContinue ? "continue" : experience.end_date ? formatDate(experience.end_date) : ' N/A '}`,
                    style: 'dates'
                },
                !_.isEmpty(experience.description) ? { text: 'Summary ', style: 'subTitle' } : {},
                !_.isEmpty(experience.description) ? { text: experience.description, style: 'subTitleBodyText' } : {}
            ])
...
function formatProjects(title, project_history) {
    ...
    project_history.forEach((project, index) => {
        ...
        data.push(...[
            ...
            {
                text: `${project.start_date ? formatDate(project.start_date) : ' N/A '} - ${project.isContinue ? "continue" : project.end_date ? formatDate(project.end_date) : ' N/A '}`,
                style: 'dates'
            },
            !_.isEmpty(project.description) ? { text: 'Project Description', style: 'subTitle' } : {},
            !_.isEmpty(project.description) ? { text: project.description, style: 'subTitleBodyText' } : {}
        ])
    })
}
    ...
    if (!_.isEmpty(work_history)) {
        ...
        work_history.forEach((experience, index) => {
            ...
            data.push(...[
                ...
                !_.isEmpty(experience.duties_responsibilities) ? { text: 'Duties & Responsibilities', style: 'subTitle' } : {},
                !_.isEmpty(experience.duties_responsibilities) ? { text: experience.duties_responsibilities, style: 'subTitleBodyText' } : {},
                !_.isEmpty(experience.achievements) ? { text: 'Achievements', style: 'subTitle' } : {},
                !_.isEmpty(experience.achievements) ? { text: experience.achievements, style: 'subTitleBodyText' } : {},
    ...
    if (!_.isEmpty(education_history)) {
        ...
        education_history.forEach((degree, index) => {
            ...
            data.push(...[
                ...
                {
                    text: `${degree.start_date ? formatDate(degree.start_date) : ' N/A '} - ${degree.isContinue ? "continue" : degree.end_date ? formatDate(degree.end_date) : ' N/A '}`,
                    style: 'dates'
                },
                !_.isEmpty(degree.grade) ? { text: 'Grade ' + degree.grade, style: 'subTitle' } : {},
                !_.isEmpty(degree.course_summary) ? { text: 'Course Summary', style: 'subTitle' } : {},
                !_.isEmpty(degree.course_summary) ? { text: degree.course_summary, style: 'subTitleBodyText' } : {},
...
function formatBasicInformation(basic_information) {
    const formattedData = [
        ...
        {
            text: !_.isEmpty(basic_information.tagline) ? basic_information.tagline : null,
            style: 'tagline'
        },
        {
            text: `${!_.isEmpty(basic_information.mobile_number) ? basic_information.mobile_number : ''} ${!_.isEmpty(basic_information.email) ? ' | ' + basic_information.email : ''}`,
            style: 'contacts'
        },
        {
            text: `${!_.isEmpty(basic_information.street) ? basic_information.street : ''} ${!_.isEmpty(basic_information.city) ? ' | ' + basic_information.city : ''} ${!_.isEmpty(basic_information.state_province_region) ? ' | ' + basic_information.state_province_region : ''}`,
            style: 'address'
        },
        {
            text: `${!_.isEmpty(basic_information.linkedIn) ? 'LinkedIn : ' + basic_information.linkedIn + '\n' : ''} ${!_.isEmpty(basic_information.gitHub) ? 'GitHub : ' + basic_information.gitHub + '\n' : ''} ${!_.isEmpty(basic_information.bitBucket) ? 'BitBucket : ' + basic_information.bitBucket + '\n' : ''} ${!_.isEmpty(basic_information.youTube) ? 'YouTube : ' + basic_information.youTube + '\n' : ''} ${!_.isEmpty(basic_information.gitLab) ? 'GitLab : ' + basic_information.gitLab + '\n' : ''}`,
            style: 'media'
        }
