...
export default function ItemAwardHonor({
  award,
  deleteAward,
    ...
    >
      <Box sx={{ flexGrow: 1 }}>
        <Typography sx={{ m: 'auto 0' }} variant="body2" color="text.secondary">
          {award.award_date ? formatDate(award.award_date) : null}
        </Typography>

        {!_.isEmpty(award.name) ? (
      ...
      >
        <Button
          ...
          size="small"
          onClick={() => openFormForAwardHonorEdit(award._id)}
        >
          Edit
        </Button>
        <Button
