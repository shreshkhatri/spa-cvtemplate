...
export default function ItemCouncil({
  council,
  deleteCouncil,
    ...
    >
      <Box sx={{ flexGrow: 1 }}>
        <Typography sx={{ m: 'auto 0' }} variant="body2" color="text.secondary">
          {council.start_date ? formatDate(council.start_date) : null}{' '}
          {council.isContinue
            ? ' - continue'
            : council.end_date
        ...
        <Typography display="inline" sx={{ fontWeight: 'bold' }}>
          {' '}
          {capitalizeWords(council.name)}{' '}
        </Typography>
        {!_.isEmpty(council.city) ? (
          <Typography display="inline">
            , {capitalizeWords(council.city)}{' '}
          </Typography>
        ) : null}
        {!_.isNull(council.country) ? (
          <Typography display="inline">
            , {capitalizeWords(council.country.label)}{' '}
          </Typography>
        ) : null}
      </Box>
      <Box
      ...
      >
        <Button
          ...
          size="small"
          onClick={() => openFormForCouncilEdit(council._id)}
        >
          Edit
        </Button>
        <Button
