...
export default function ItemEditorial({
  experience,
    ...
    >
      <Box sx={{ flexGrow: 1 }}>
        <Typography sx={{ m: 'auto 0' }} variant="body2" color="text.secondary">
          {experience.start_date ? formatDate(experience.start_date) : null}
        ...
        <Typography display="inline" sx={{ fontWeight: 'bold' }}>
          {capitalizeWords(experience.role)},{' '}
          {capitalizeWords(experience.organization)}
          {!_.isEmpty(experience.city)
            ? ', ' + capitalizeWords(experience.city)
            : null}
          {!_.isNull(experience.country)
            ? ', ' + experience.country.label
            : null}
        </Typography>
      ...
      >
        <Button
          ...
          size="small"
          onClick={() => openFormForEditorialExperienceEdit(experience._id)}
        >
          Edit
        </Button>
        <Button
          ...
          size="small"
          onClick={() => deleteEditorialExperience(experience._id)}
        >
          Delete
        </Button>
      </Box>
