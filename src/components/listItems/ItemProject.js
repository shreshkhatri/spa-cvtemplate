    ...
    >
      <Box sx={{ flexGrow: 1, flexWrap: 'wrap' }}>
        ...
        <Typography display="inline" sx={{ fontWeight: 'bold' }}>
          {capitalizeWords(project.project_title)} ,{' '}
          {capitalizeWords(project.designation)}
          {project.city ? ', ' + capitalizeWords(project.city) : null}
          {project.country ? `, ${project.country.label}` : null}
        </Typography>
      ...
      >
        <Button
          ...
          size="small"
          onClick={() => openFormForProjectEdit(project._id)}
        >
          Edit
        </Button>
        <Button
